...
def _create_reader(numbering, content_types, relationships, styles, docx_file, files):
    ...
    def paragraph(element):
        ...
        def create_paragraph(style, children):
            paragraph_numbering = _read_numbering_properties(
                paragraph_style_id=style[0],
                element=pr_element,
            )
            num_id = _read_effective_num_id(pr_element, style[0])
            list_id, start_override = _read_list_identity(num_id, paragraph_numbering)
    ...
    def _read_effective_num_id(element, paragraph_style_id):
        # An explicit w:numPr on the paragraph wins; otherwise numbering can
        # come from the paragraph style's own w:numPr (common in documents
        # generated by tools such as FrameMaker).
        num_id = element.find_child_or_null("w:numId").attributes.get("w:val")
        if num_id is not None:
            return num_id
        if paragraph_style_id is not None:
            style = styles.find_paragraph_style_by_id(paragraph_style_id)
            if style is not None:
                return style.num_id
        return None

    def _read_list_identity(num_id, paragraph_numbering):
        # Word keeps one counter per abstract num, shared by every num that
        # references it, so list continuation is keyed by the abstract num id.
        # A num's startOverride restarts that shared counter.
        if num_id is None or paragraph_numbering is None:
            return num_id, None
        abstract_num_id = numbering.find_abstract_num_id(num_id)
        if abstract_num_id is None:
            return num_id, None
        start_override = numbering.find_start_override(num_id, paragraph_numbering.level_index)
        return "abstract-" + abstract_num_id, start_override
