...
class ParagraphTests(object):
    ...
    def test_paragraph_with_deleted_paragraph_mark_preserves_alignment(self):
        body_xml = [
            xml_element("w:p", {}, [
                xml_element("w:pPr", {}, [
                    xml_element("w:jc", {"w:val": "center"}),
                    xml_element("w:rPr", {}, [
                        xml_element("w:del"),
                    ]),
                ]),
                _run_element_with_text("Centered content"),
            ]),
        ]

        result = _read_and_get_document_xml_elements(body_xml)
        ...
        assert_that(result, is_sequence(
            all_of(
                ...
                has_attrs(
                    alignment="center",
                    children=is_sequence(
                        documents.run([documents.text("Centered content")]),
                    ),
                ),
            ),
        ))
...
class ImageTests(object):
    ...
    def _read_embedded_image(self, element, relationships=None):
        return self._read_embedded_images(element, relationships=relationships)[0]

    def _read_embedded_images(self, element, relationships=None):
        relationships = Relationships([
